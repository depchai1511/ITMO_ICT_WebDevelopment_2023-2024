...
class MyHTTPServer:
    ...
    def serve_forever(self):
        serv_sock = socket.socket(
            socket.AF_INET,  # Семейство протоколов адресации Интернета IPv4
            socket.SOCK_STREAM,  # Потоковый сокет
            proto=0
        )

        try:
            serv_sock.bind((self._host, self._port))  # Привязка сокета к адресу и порту
            serv_sock.listen()  # Прослушивание входящих соединений

            while True:
                conn, _ = serv_sock.accept()  # Принятие входящего соединения
                try:
                    self.serve_client(conn)  # Обслуживание клиента
                except Exception as e:
                    print('Client serving failed', e)
        finally:
            serv_sock.close()  # Закрытие сокета

    def serve_client(self, conn):
        try:
            req = self.parse_request(conn)  # Разбор HTTP-запроса
            if req.method == 'GET':
                resp = self.handle_get_request(req)  # Обработка GET-запроса
            elif req.method == 'POST':
                resp = self.handle_post_request(req)  # Обработка POST-запроса
            else:
                resp = self.create_response(405, 'Method Not Allowed', 'Only GET and POST methods are supported.')

            self.send_response(conn, resp)  # Отправка HTTP-ответа
    ...
    def parse_request(self, conn):
        ...
        params = (
            {p.split("=")[0]: p.split("=")[1] for p in path.split("?")[1].split("&")}  # Извлечение параметров из строки запроса
            if "?" in path
            else None
        )
        print(params)
...
class HTTPRequest:
    def __init__(self, method, path, headers, params):
        ...
        self.path = path  # Путь запроса
        self.headers = headers  # Заголовки запроса
        self.params = params  # Параметры запроса
...
![Пример задания 5_1](image/task5_get1.png)
![Пример задания 5_2](image/task5_get2.png)